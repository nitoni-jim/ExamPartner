...
def mark_user_paid_by_identifier(
    ...
    try:
        ...
        # --- plan/expiry logic (Founding: first 100 paid users) ---
        # Note: "Founding" is a privilege flag, not a permanent discount.
        # Access is controlled by paid_until (30 days from payment, extended on renewal).
        cur.execute("SELECT is_founding, plan, paid_until FROM users WHERE id = ?", (user_id,))
        srow = cur.fetchone() or {}
        is_founding = bool(srow.get("is_founding") or False)
        plan = (srow.get("plan") or "free").strip() or "free"
        paid_until = srow.get("paid_until")

        # Qualify for founding only on the ₦1,000 payment and only for the first 100 users.
        FOUNDING_CAP = 100
        FOUNDING_AMOUNT_KOBO = 100_000  # ₦1,000
        qualifies_for_founding = (amount_kobo == FOUNDING_AMOUNT_KOBO)

        if (not is_founding) and qualifies_for_founding:
            cur.execute("SELECT COUNT(*) AS n FROM users WHERE is_founding = TRUE")
            crow = cur.fetchone() or {}
            used = int(crow.get("n") or 0)
            if used < FOUNDING_CAP:
                is_founding = True
                plan = "founding"
                cur.execute(
                    "UPDATE users SET is_founding = TRUE, plan = ? WHERE id = ?",
                    (plan, user_id),
                )

        # Extend expiry: paid_until = max(now, existing paid_until) + 30 days
        now = datetime.now(timezone.utc)
        base = paid_until if (paid_until and paid_until > now) else now
        new_paid_until = base + timedelta(days=30)

        # Mark paid + set expiry
        cur.execute(
            "UPDATE users SET is_paid = TRUE, paid_until = ? WHERE id = ?",
            (new_paid_until, user_id),
        )
